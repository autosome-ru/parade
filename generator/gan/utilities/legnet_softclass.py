...
class AdaptiveAvgMaxPool1d(nn.Module):
    ...
    def forward(self, x):
        avg_x = F.adaptive_avg_pool1d(x, self.output_size)
        max_x = F.adaptive_max_pool1d(x, self.output_size)
        x = torch.concat((avg_x, max_x), dim=-2)
        return x
...
class LegNetClassifier(nn.Module):
    ...
    se_layer : bool, optional
        Use SE layer. The default is True,
    ...
    def __init__(self,
        ...
        self.register_buffer('bins', torch.arange(start=1, end=5, step=1, requires_grad=False))
        ...
        self.avgpooling = nn.Sequential(
            AdaptiveAvgMaxPool1d(1),
            nn.Flatten(),
        )
        
        self.pooled_mapper_size = self.mapper_size * 2

        if self.linear_sizes is not None:
            first_linear = nn.Sequential(
                nn.Dropout(0.1),
                nn.Linear(self.pooled_mapper_size, self.linear_sizes[0]),
            ...
            self.linear = nn.Sequential(
                nn.Linear(self.pooled_mapper_size, self.out_channels),
                final_activation()
            )
